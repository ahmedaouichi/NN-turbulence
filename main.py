...
def main():
    ...
    eps = 1e-1*np.ones([DIM_Y, DIM_Z]) ## For now using epsilon=1 everywhere
    ...
    #stresstensor, tensorbasis, stresstensor = shuffle(stresstensor, tensorbasis, stresstensor)
    
#    for ii in range(len(tensorbasis[:,0,0])-1):
#        std = np.std(tensorbasis[ii,:,:])
#        mean = np.mean(tensorbasis[ii,:,:])
#        print(mean, std)
#        if std!=0:
#            tensorbasis[ii,:,:] = (tensorbasis[ii,:,:] - mean) / std
    
    print('--> Train network')
    neural_network.model.fit([tensorbasis, eigenvalues], stresstensor, batch_size = 500, epochs = 1000, verbose = 1, shuffle=True, validation_split=0.2)
    ############# Visualization ###############################################
